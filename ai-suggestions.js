(() => {
  ...
  function recommendationFor(row) {
    ...
    if (stage === 'Blocked') return {label:'Plan next action',kind:'blocked',stage:'Scheduled',technician:assigned,reason:'Review the blocker and prepare the job to return to the schedule.'};
  ...
  function openReview(row, proposal) {
    ...
    let changeRows = [];
    let fieldHtml = '';
    if (proposal.kind === 'assign' || proposal.kind === 'blocked') {
    ...
    const isAssignment = proposal.kind === 'assign' || proposal.kind === 'blocked';
    scrim.querySelector('.ai-review').classList.toggle('ai-review-wide', isAssignment);
    if (isAssignment) {
    ...
    document.getElementById('aiApprove').textContent = proposal.kind === 'complete' ? 'Approve and complete' : (proposal.kind === 'summary' && role === 'technician' ? 'Add summary' : 'Approve changes');
  ...
  document.getElementById('aiApprove').addEventListener('click', () => {
    ...
    closeReview();
    confirmation.textContent = `${proposal.label} approved. Shared job views updated.`;
